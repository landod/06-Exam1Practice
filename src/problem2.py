...
Authors: David Mutchler, Vibha Alangar, Valerie Galluzzi, Mark Hays,
         Amanda Stouder, their colleagues and Owen Land.
...
def problem2b(rect, n, delta, win):
    ...
    # ------------------------------------------------------------------
    rect.attach_to(win)
    point = rect.get_upper_right_corner()
    point2 = rect.get_lower_left_corner()
    for i in range(n - 1):
        point = rg.Point(point.x + delta, point.y - delta)
        point2 = rg.Point(point2.x - delta, point2.y + delta)
        rect = rg.Rectangle(point, point2)
        rect.attach_to(win)
        win.render()
